...
func (doc PluginDocument) GetRST() string {
	...
	// add index
	original_plugin_kind := doc.PackageName
	plugin_kind := doc.PackageName
	if plugin_kind == "format" {
		// fix nomenclature
		plugin_kind = "formatter"
	}

	// Print heading
	result += fmt.Sprintf(".. title:: %s\n", doc.PluginName)
	result += fmt.Sprintf(".. gollum:plugin:: %s.%s\n", original_plugin_kind, doc.PluginName)
	...
	for _, parentName := range sortInheritedKeys(doc.InheritedMetadata) {
		...
		result += doc.InheritedMetadata[parentName].getRST(false, 0, parentName)
	}
	...
	for _, parentName := range sortInheritedKeys(doc.InheritedParameters) {
		...
		result += doc.InheritedParameters[parentName].getRST(true, 0, parentName)
	}
